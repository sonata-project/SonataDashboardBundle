...
Bug fixes            | Not on legacy            | Patch           | `master`   | |
...
Notes:
  * Branch `master` is the branch of the **latest stable** patch releases and
  ...
  **No bug fix will be accepted here**, except if the bug fix concerns `master` and **only this one**.
  Bug fixes merged on `master` will be ported on other branches by fallback merging.
