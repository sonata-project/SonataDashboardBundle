 ...
 *
 * generated on: Mon Jun 13 2016 15:41:31 GMT+0200 (CEST)
 * revision:     e4d37a1503dc6e1f49569677e0b381cb139b1a86
 *
 */
/**
 *
...
Sonata.Dashboard = {
    ...
    /**
     * Collection of blocks found on the dashboard
     *
     * @var array
     */
    blocks: [],
    ...
    /**
     * Collection of containers found on the dashboard
     *
     * @var array
     */
    containers: [],
    ...
    /**
     * Initialize Dashboard editor mode
     */
    ...
    initInterface: function() {
        jQuery('#dashboard-action-enabled-edit').change(jQuery.proxy(this.toggleEditMode, this));
        jQuery('#dashboard-action-save-position').click(jQuery.proxy(this.saveBlockLayout, this));
    },
    ...
    buildBlockData: function() {
        ...
        // sort items on dashboard, parent and position
        data.sort(function(a, b) {
            if (a.dashboard_id == b.dashboard_id) {
    ...
    buildSingleBlockData: function(original) {
        var block, id, parent, parentId, dashboardId, previous, position;
        ...
        // retrieve root's dashboard (because a root container cannot be moved)
        root = this.findRootContainer(block);
        if (!root) {
            this.log('Block '+id+' has no root but has a parent, should never happen!');
            return;
        }
        dashboardId = jQuery(root).attr('data-dashboard-id');
    ...
    buildDiffBlockData: function(previousData, newData) {
        ...
        jQuery.map(previousData, function(previousItem, index) {
            ...
            found = jQuery.grep(newData, function(newItem, index) {
                ...
                if (previousItem.position != newItem.position || previousItem.parent_id != newItem.parent_id || previousItem.dashboard_id != newItem.dashboard_id) {
                    return true;
                }
            });
    ...
    saveBlockLayout: function(event) {
        ...
        jQuery.each(diff, jQuery.proxy(function(item, block) {
            this.log('Update block '+block.id+ ' (Dashboard '+block.dashboard_id+'), parent '+block.parent_id+', at position '+block.position+')');
        }, this));
